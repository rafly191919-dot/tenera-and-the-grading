...
window.addEventListener('error', (event) => {
  const msg = event?.error?.message || event.message || 'Terjadi error pada aplikasi.';
  if (!state.currentUser) showLoginError(msg);
  else setStatus(msg, 'error');
  console.error(event.error || event.message || event);
});
window.addEventListener('unhandledrejection', (event) => {
  const reason = event?.reason;
  const msg = reason?.message || String(reason || 'Terjadi error async pada aplikasi.');
  if (!state.currentUser) showLoginError(msg);
  else setStatus(msg, 'error');
...
function supplierExistsByName(name, excludeId = '') { return state.suppliers.some((s) => s.id !== excludeId && normalizeText(s.name) === normalizeText(name)); }
...
function bind(node, eventName, handler, options) {
...
async function ensureDefaultSuppliers() {
  ...
  if (snap.exists() && Object.keys(snap.val() || {}).length) return;
...
function setupRolePicker() {
  const buttons = Array.from(document.querySelectorAll('.role-pick, .role-btn'));
  buttons.forEach((button) => {
    button.addEventListener('click', () => {
      buttons.forEach((btn) => btn.classList.remove('active'));
      button.classList.add('active');
      const role = button.dataset.role;
      if (el('loginEmail')) el('loginEmail').value = ROLE_EMAILS[role] || '';
      showLoginError('');
    });
  });
  if (el('loginEmail')) {
    if (!el('loginEmail').value) el('loginEmail').value = ROLE_EMAILS.staff;
    el('loginEmail').setAttribute('autocomplete', 'username');
  }
  if (el('loginPassword')) el('loginPassword').setAttribute('autocomplete', 'current-password');
}
...
async function handleLoginSubmit(event) {
  ...
  try {
    ...
    showLoginError(messageMap[error.code] || error.message || 'Login gagal.');
  }
}
...
function fillStatic() {
  ...
  const selectedGradingSupplier = gradingSelect.value;
  const selectedRekapSupplier = el('rekapGradingSupplier').value;
  const selectedRekapDataSupplier = el('rekapDataSupplier').value;
  const selectedWaSupplier = el('waSupplier').value;
  ...
  if (active.some((supplier) => supplier.name === selectedGradingSupplier)) gradingSelect.value = selectedGradingSupplier;
  if (state.suppliers.some((supplier) => supplier.name === selectedRekapSupplier)) el('rekapGradingSupplier').value = selectedRekapSupplier;
  if (state.suppliers.some((supplier) => supplier.name === selectedRekapDataSupplier)) el('rekapDataSupplier').value = selectedRekapDataSupplier;
  if (state.suppliers.some((supplier) => supplier.name === selectedWaSupplier)) el('waSupplier').value = selectedWaSupplier;
...
function supplierStats(rows = state.grading) {
  const map = {};
  rows.filter((row) => row && row.supplier).forEach((row) => {
    if (!map[row.supplier]) {
      map[row.supplier] = {
        name: row.supplier,
        count: 0,
...
function driverStats(rows = state.grading) {
  const map = {};
  rows.filter((row) => row && row.driver).forEach((row) => {
    if (!map[row.driver]) {
...
async function handleSupplierSubmit(event) {
  ...
  try {
    if (supplierExistsByName(name, supplierId)) {
...
function bindEvents() {
  ...
  setupRolePicker();
  bind(el('loginForm'), 'submit', handleLoginSubmit);
  bind(el('logoutBtn'), 'click', () => signOut(auth));
  document.querySelectorAll('.menu-item').forEach((button) => bind(button, 'click', () => switchPage(button.dataset.page)));
  bind(el('menuToggle'), 'click', () => toggleSidebar(!el('app').classList.contains('sidebar-open')));
  bind(el('mobileOverlay'), 'click', () => toggleSidebar(false));
  bind(window, 'resize', () => { if (window.innerWidth > 900) toggleSidebar(false); });

  bind(el('gradingDriver'), 'input', (event) => { historyHint(event.target.value.trim()); renderGradingLive(); });
  bind(el('gradingForm'), 'input', renderGradingLive);
  bind(el('tdForm'), 'input', renderTDLive);
  bind(el('gradingForm'), 'submit', handleGradingSubmit);
  bind(el('tdForm'), 'submit', handleTDSubmit);

  bind(el('resetGradingBtn'), 'click', () => {
    const form = el('gradingForm');
    if (!form) return;
    form.reset();
    form.elements.totalBunches.value = 0;
    form.querySelectorAll('.cat').forEach((input) => { input.value = 0; });
    if (el('driverHint')) el('driverHint').textContent = 'Belum ada histori sopir.';
    renderGradingLive();
  });
  bind(el('resetTDBtn'), 'click', () => {
    const form = el('tdForm');
    if (!form) return;
    form.reset();
  ...
  ['rekapGradingSearch'].forEach((id) => bind(el(id), 'input', renderRekapGrading));
  ['rekapGradingSupplier', 'rekapGradingStart', 'rekapGradingEnd'].forEach((id) => bind(el(id), 'change', renderRekapGrading));
  ['rekapTDSearch'].forEach((id) => bind(el(id), 'input', renderRekapTD));
  ['rekapTDStart', 'rekapTDEnd'].forEach((id) => bind(el(id), 'change', renderRekapTD));
  bind(el('rekapDataRunBtn'), 'click', renderRekapData);
  bind(el('rekapDataResetBtn'), 'click', () => {
    if (el('rekapDataStart')) el('rekapDataStart').value = '';
    if (el('rekapDataEnd')) el('rekapDataEnd').value = '';
    if (el('rekapDataSupplier')) el('rekapDataSupplier').value = '';
    if (el('rekapDataDriver')) el('rekapDataDriver').value = '';
    renderRekapData();
  });
  bind(el('sheetGradingSearch'), 'input', renderSheetGrading);
  bind(el('sheetTDSearch'), 'input', renderSheetTD);
  bind(el('performanceMode'), 'change', renderPerformance);
  bind(el('performanceView'), 'change', renderPerformance);
  bind(el('supplierForm'), 'submit', handleSupplierSubmit);
  bind(el('resetSupplierBtn'), 'click', () => {
  ...
  bind(document, 'focusout', handleSheetFocusOut, true);
  bind(el('closeModalBtn'), 'click', () => el('detailModal')?.classList.remove('open'));
  bind(el('closeWaModalBtn'), 'click', () => el('waModal')?.classList.remove('open'));
  bind(el('detailModal'), 'click', (event) => { if (event.target === el('detailModal')) el('detailModal').classList.remove('open'); });
  bind(el('waModal'), 'click', (event) => { if (event.target === el('waModal')) el('waModal').classList.remove('open'); });
  bind(document, 'keydown', (event) => { if (event.key === 'Escape') { el('detailModal')?.classList.remove('open'); el('waModal')?.classList.remove('open'); toggleSidebar(false); } });
  bind(el('confirmWaBtn'), 'click', openWhatsApp);
  ['waStart', 'waEnd', 'waSupplier', 'waMode'].forEach((id) => bind(el(id), 'input', updateWaPreview));
  ['waStart', 'waEnd', 'waSupplier', 'waMode'].forEach((id) => bind(el(id), 'change', updateWaPreview));
  bind(el('sendGradingSummaryBtn'), 'click', () => openWaModal('grading-summary', 'Kirim Ringkasan Grading'));
  bind(el('sendGradingDetailBtn'), 'click', () => openWaModal('grading-detail', 'Kirim Detail Grading'));
  bind(el('sendTDSummaryBtn'), 'click', () => openWaModal('td-summary', 'Kirim Ringkasan Tenera Dura'));
  bind(el('sendTDDetailBtn'), 'click', () => openWaModal('td-detail', 'Kirim Detail Tenera Dura'));
  bind(el('waFilteredGradingBtn'), 'click', () => openWaModal('grading-summary', 'Kirim Ringkasan Grading'));
  bind(el('waFilteredTDBtn'), 'click', () => openWaModal('td-summary', 'Kirim Ringkasan Tenera Dura'));
  bind(el('exportGradingBtn'), 'click', exportGrading);
  bind(el('exportTDBtn'), 'click', exportTD);
  bind(el('globalSearch'), 'input', (event) => {
    const value = event.target.value;
    if (el('rekapGradingSearch')) el('rekapGradingSearch').value = value;
    if (el('rekapTDSearch')) el('rekapTDSearch').value = value;
    if (el('sheetGradingSearch')) el('sheetGradingSearch').value = value;
    if (el('sheetTDSearch')) el('sheetTDSearch').value = value;
    renderRekapGrading();
    renderRekapTD();
    renderSheetGrading();
    renderSheetTD();
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!user) {
    ...
    toggleSidebar(false);
    showLoginError('');
    if (el('loginEmail') && !el('loginEmail').value) el('loginEmail').value = ROLE_EMAILS.staff;
    return;
  }
